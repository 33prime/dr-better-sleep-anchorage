...
interface Props {
  ...
  size?: number;
  /** Color treatment for the "Dr" prefix: white on dark surfaces, midnight on light. */
  tone?: 'onDark' | 'onLight';
  style?: CSSProperties;
}
 ...
 *  - The product name renders in Restful Teal (#43BACA), the product's signature accent.
 *  - Nunito Black (900) for maximum display impact.
 */
export function Wordmark({ size = 22, tone = 'onDark', style }: Props) {
  const drColor = tone === 'onDark' ? 'var(--night-text-1)' : 'var(--text-primary)';
